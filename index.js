...
async function doTheJob () {
  try {
    ...
    if (!fetchedFromLibeApi.length) {
      console.log('Nothing new to save. Exiting now.')
      return process.exit(0)
    }

    const newRawSheetData = await prepareNewRawSheetData(fetchedFromLibeApi, initSheetData)
    console.log(newRawSheetData)
    console.log('\n')
...
async function fetchNewEntriesInLibeApi (initEntries = [], currUrl = libeApiBaseUrl, results = [], page = 0) {
  ...
  const pageData = await fetchApiPage(currUrl)
  ...
  console.log(`Found ${pageResults.length} entries, and ${pageResultsThatAreNew.length} new ones.\n`)
